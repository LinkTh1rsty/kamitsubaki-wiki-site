---
locale: ja
translationKey: koko_album_prayer
title: "prayer"
romanizedTitle: "prayer"
artist: "幸祜"
type: "スタジオアルバム"
description: "幸祜の正式リリース『prayer』。全曲順を確認し、各楽曲ページへリンクしています。"
releaseDate: "2022-02-26"
label: "℗ KAMITSUBAKI RECORD"
trackCount: 13
duration: "52:13"
categoryTitle: "アルバム"
categorySubtitle: ALBUMS
categoryOrder: 1
itemOrder: 1645862400000
image: /images/albums/koko/prayer.jpg
officialLinks:
  - label: "公式ディスコグラフィー"
    href: https://kamitsubaki.jp/disco_category/koko/
  - label: "Apple Music Japan"
    href: https://music.apple.com/jp/album/prayer/1688671440?uo=4
tracks:
  -
    number: "1"
    title: "白昼夢"
    duration: "04:23"
    songId: "koko/originals/白昼夢-daydream"
  -
    number: "2"
    title: "LIT"
    duration: "03:44"
    songId: "koko/originals/lit"
  -
    number: "3"
    title: "花と蜜"
    duration: "04:09"
    songId: "koko/originals/花と蜜-flowers-and-nectar"
  -
    number: "4"
    title: "瞑目"
    duration: "03:22"
    songId: "koko/originals/瞑目-closing-ones-eyes"
  -
    number: "5"
    title: "錘"
    duration: "04:34"
    songId: "koko/originals/錘-weight"
  -
    number: "6"
    title: "the last bullet"
    duration: "03:46"
    songId: "koko/originals/the-last-bullet"
  -
    number: "7"
    title: "harmony"
    duration: "03:48"
    songId: "koko/originals/harmony"
  -
    number: "8"
    title: "レイヴン・フリージア"
    duration: "03:57"
    songId: "koko/originals/レイヴンフリージア-raven-freesia"
  -
    number: "9"
    title: "夜光を呼ぶ"
    duration: "04:42"
    songId: "koko/originals/夜光を呼ぶ-recall-me"
  -
    number: "10"
    title: "この世界に口づけを"
    duration: "03:20"
    songId: "koko/originals/この世界に口づけを-kiss-this-world"
  -
    number: "11"
    title: "此処へ"
    duration: "04:42"
    songId: "koko/originals/此処へ-koko-e"
  -
    number: "12"
    title: "閃光の彼方"
    duration: "03:52"
    songId: "koko/originals/閃光の彼方-beyond-the-flash"
  -
    number: "13"
    title: "bliss"
    duration: "03:55"
    songId: "koko/originals/bliss"
---

## 作品概要

『prayer』は本カタログに収録する正式リリースです。発売日、曲順、収録時間、参加アーティストは公式ディスコグラフィーとApple Music日本版を照合しています。

## トラックとバージョン

全13トラックを掲載。各トラックは個別の楽曲・バージョンページへリンクします。再収録は同一ページを再利用し、ライブ、リミックス、リアレンジ版は別ページとしています。

## 資料方針

ジャケットはApple Musicの最高原生解像度を優先し、フィジカル限定作品は公式商品画像を使用します。

## 出典

- [公式ディスコグラフィー](https://kamitsubaki.jp/disco_category/koko/)
- [Apple Music](https://music.apple.com/jp/album/prayer/1688671440?uo=4)
- [VGMdb 幸祜](https://vgmdb.net/artist/53600)
